...
- **Worktree cleanup policy** on `story.complete` / abandon (remove vs. retain) — **still open**; default v1: **retain** on complete (worktree holds the PR branch until merged), remove on explicit abandon. Confirm when the PR-merge path is built.
